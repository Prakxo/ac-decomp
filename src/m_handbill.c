...
static int mHandbill_Put_String_FREE(u8* str, int buf_size, int start_idx, int str_len, int str_no, int fill_type) {
  ...
  if (article != mIN_ARTICLE_NONE) {
    ...
    cut_len = mHandbill_MoveDataCut(str, buf_size, start_idx + free_str_len + 1, start_idx, cut_len, mHandbill_FILL_NONE); // Create free space for the article
...
static int mHandbill_Cut_Article(u8* buf, int buf_size, int start_idx, int str_len, int fill_type) {
  ...
  return mHandbill_MoveDataCut(buf, buf_size, start_idx, start_idx + code_size, str_len, mHandbill_FILL_NONE); // remove the control code characters
}
...
static void mHandbill_CheckSuperStringBorderAndCopy(u8* dst, int dst_size, int* header_back_start, u8* src, int src_size) {
  int src_pos;
  int lines;
  int dst_pos;
  u8* src_p = src;
...
/* @nonmatching TODO: minor regswap here */
static void mHandbill_Load_SuperStringFromRom(u8* buf, int buf_size, int* header_back_start, int super_no) {
  ...
  if (super_no >= 0 && super_no < mHandbill_MAIL_NUM) {
    ...
    if (super_address != 0 && super_size != 0) {
      ...
      bcopy(super_buf, buf, buf_size < 43 ? buf_size : 43);
...
extern void mHandbill_Load_HandbillFromRom2(u8* header, int header_size, int* header_back_start, u8* footer, int footer_size, u8* body, int mail_no) {
  mHandbill_load_init();
...
static int mHandbillzDMA_body_addr_set(mHandbillzDMA_c* dma_info) {
  ...
  mMsg_Get_BodyParam(mHandbillz_table_pos[type], mHandbillz_data_pos[type], num, &dma_info->dma_addr, &dma_info->dma_size);

...
JKRAramBlock *JKRAram::mainRamToAram(u8 *buf, u32 address, u32 alignedSize, JKRExpandSwitch expandSwitch, u32 fileSize, JKRHeap *heap, int id)
{
    JKRAramBlock *block = nullptr;
    checkOkAddress(buf, address, nullptr, 0);
    if (expandSwitch == EXPAND_SWITCH_DECOMPRESS)
    {
        expandSwitch = (JKRCheckCompressed(buf) == JKRCOMPRESSION_NONE) ? EXPAND_SWITCH_DEFAULT : EXPAND_SWITCH_DECOMPRESS;
    }
    if (expandSwitch == EXPAND_SWITCH_DECOMPRESS)
    {
...
u8 *JKRAram::aramToMainRam(u32 address, u8 *buf, u32 srcSize, JKRExpandSwitch expandSwitch, u32 p5, JKRHeap *heap, int id, u32 *pSize)
{
...
u8 *JKRAram::aramToMainRam(JKRAramBlock *block, u8 *buf, u32 bufSize, u32 alignedBlockSize, JKRExpandSwitch expandSwitch, u32 p6, JKRHeap *heap, int id, u32 *pSize)
{
    int compression = JKRCOMPRESSION_NONE;
    if (pSize)
        *pSize = 0;

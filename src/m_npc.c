...
extern int mNpc_CheckCmpAnimalPersonalID(AnmPersonalID_c* pid0, AnmPersonalID_c* pid1) {
  ...
  if (
    (pid0 != NULL && pid1 != NULL) &&
    (pid0->npc_id == pid1->npc_id) &&
    (pid0->name_id == pid1->name_id) &&
    (pid0->land_id == pid1->land_id) &&
...
extern void mNpc_SetRemoveExp(Animal_c* animal, u16 remove_exp) {
  animal->remove_info = ((((animal->remove_info >> 13) & 0b111) >> 13) | remove_exp) << 13; // what the... "official" impl from DnM
}
#else
extern void mNpc_SetRemoveExp(Animal_c* animal, u16 remove_exp) {
...
extern void mNpc_SetParentName(Animal_c* animal, PersonalID_c* parent_id) {
  if (parent_id != NULL && mPr_NullCheckPersonalID(parent_id) == FALSE && mPr_NullCheckPlayerName(animal->parent_name) == TRUE) {
...
extern void mNpc_RenewalAnimalMemory() {
  ...
  for (i = 0; i < ANIMAL_NUM_MAX; i++) {
    ...
    for (j = 0; j < ANIMAL_MEMORY_NUM; j++) {
      if (mNpc_CheckFreeAnimalMemory(memory) == FALSE && mLd_CheckThisLand(memory->memory_player_id.land_name, memory->memory_player_id.land_id) == TRUE) {
        priv = Save_Get(private);

        for (k = 0; k < PLAYER_NUM; k++) {
          if (mPr_NullCheckPersonalID(&priv->player_ID) == FALSE && mPr_CheckCmpPersonalID(&priv->player_ID, &memory->memory_player_id) == TRUE) {
            break;
          }
        ...
        if (chosen_mem != NULL) {
          if (chosen_mem->letter_info.exists == memory->letter_info.exists) {
            if (chosen_mem->friendship > memory->friendship) {
              ...
              if (lbRTC_IsOverTime(&chosen_mem->last_speak_time, &memory->last_speak_time) == lbRTC_LESS) {
                chosen_mem = memory;
                free_idx = i;
              }
            }
...
static int mNpc_GetAnimalMemoryFriend_Land_Sex(Anmmem_c* memory, int num, int sex) {
  ...
  for (i = 0; i < num; i++) {
    if (mNpc_CheckFreeAnimalMemory(memory) == FALSE) {
      ...
      if (priv_idx != -1 && Save_Get(private[priv_idx]).gender == sex && mNpc_SelectBestFriend(&best_friend, memory, &best_friendship) == TRUE) {
        res = i;
      }
    }
...
extern int mNpc_CheckFreeAnimalInfo(Animal_c* animal) {
  ...
  if ((animal->home_info.block_x == 0xFF && animal->home_info.block_z == 0xFF) && ITEM_NAME_GET_TYPE(animal->id.npc_id) != NAME_TYPE_NPC) {
    res = TRUE;
  }
    ...
    for (j = 0; j < ANIMAL_NUM_MAX; j++) {
      ...
      if (!mNpc_CheckFreeAnimalPersonalID(&animal->id)) {
        for (npc_max = 0; npc_max < count; npc_max++) {
          if (((bitfield >> npc_max) & 1) == 1 && !mNpc_CheckCmpAnimalPersonalID(&animal->id, &pids[npc_max])) {
            i++;
          }
        }
...
static int mNpc_CheckMailChar(u8 c) {
  if (
    ...
    (c == CHAR_UNDERSCORE) ||
    (c == CHAR_HYPHEN) ||
    (c == CHAR_SYMBOL_ANNOYED) ||
    ((c >= CHAR_PERCENT) && (c <= CHAR_AT_SIGN)) ||
...
static int mNpc_SetPresentCloth(Animal_c* animal, PersonalID_c* player_id, mActor_name_t cloth) {
  ...
  if (
    animal != NULL &&
    player_id != NULL &&
    ITEM_NAME_GET_TYPE(cloth) == NAME_TYPE_ITEM1 &&
    ITEM_NAME_GET_CAT(cloth) == ITEM1_CAT_CLOTH &&
    !mNpc_CheckFreeAnimalPersonalID(&animal->id) &&
  ...
  if (mMl_get_npcinfo_from_mail_name(&anm_id, &mail->header.recipient) == TRUE) {
    ...
    if (anm_idx != -1) {
      ...
      if (mEv_CheckFirstJob() == TRUE) {
        ...
        if (
          (first_job->base.quest_kind == mQst_ERRAND_FIRSTJOB_SEND_LETTER || first_job->base.quest_kind == mQst_ERRAND_FIRSTJOB_SEND_LETTER2) &&
...
static void mNpc_GetRemailGoodData(Mail_c* mail, PersonalID_c* pid, AnmPersonalID_c* anm_id, Anmremail_c* remail, u8 foreign) {
  static int start_no[mNpc_LOOKS_NUM] = {
    0x020,
    0x040,
    0x000,
    0x060,
  ...
  static int ohter_start_no[mNpc_LOOKS_NUM] = {
    0x0E0,
    0x100,
    0x0C0,
    0x120,
  ...
  mNpc_GetHandbillz(mail, msg_no + RANDOM(32), msg_no + RANDOM(32), msg_no + RANDOM(16) + give_present * 16, msg_no + RANDOM(32), msg_no + RANDOM(32));
  mail->present = present;
}

static void mNpc_GetRemailWrongData(Mail_c* mail, PersonalID_c* pid, AnmPersonalID_c* anm_id, Anmremail_c* remail, u8 foreign) {
...
static void mNpc_GetRemailData(Mail_c* mail, PersonalID_c* pid, AnmPersonalID_c* anm_id, Anmremail_c* remail, int cond, u8 foreign) {
  static mNPC_GET_REMAIL_PROC get_remail[mNpc_LETTER_RANK_NUM] = {
...
static int mNpc_SendRemailPostOffice(PersonalID_c* pid, AnmPersonalID_c* anm_id, Anmremail_c* remail, int cond, u8 foreign) {
...
static int mNpc_CheckLetterTime(lbRTC_ymd_c* letter_date, lbRTC_time_c* rtc_time) {
  ...
  if (letter_date->day != 0xFF && (letter_date->year != rtc_time->year || letter_date->month != rtc_time->month || letter_date->day != rtc_time->day)) {
    res = TRUE;
  }
...
extern void mNpc_Remail() {
  ...
  if (mPr_NullCheckPersonalID(&priv->player_ID) == FALSE && mLd_PlayerManKindCheck() == FALSE) {
    ...
    for (i = 0; i < ANIMAL_NUM_MAX; i++) {
      if (mNpc_CheckFreeAnimalPersonalID(&animal->id) == FALSE) {
        ...
        if (mem_idx != -1) {
          ...
          if (memory->letter_info.send_reply && mNpc_CheckLetterTime(&memory->letter.date, rtc_time) == TRUE) {
            if (mNpc_SendRemailPostOffice(&priv->player_ID, &animal->id, NULL, memory->letter_info.cond, FALSE) != TRUE) {
              break;
            }
    ...
    remail = &priv->remail;
    if (remail->flags.looks != 0x7F && mNpc_SendRemailPostOffice(&priv->player_ID, &animal->id, remail, remail->flags.cond, TRUE) == TRUE) {
...
static void mNpc_LoadMailDataCommon2(Mail_c* mail, PersonalID_c* pid, AnmPersonalID_c* anm_id, mActor_name_t present, u8 paper_type, int mail_no) {
  u8 super[MAIL_HEADER2_LEN];
  u8 ps[MAIL_FOOTER2_LEN];
  int header_back_start;

  mMl_clear_mail(mail);
  mHandbill_Load_HandbillFromRom2(super, MAIL_HEADER2_LEN, &header_back_start, ps, MAIL_FOOTER2_LEN, mail->content.body, mail_no);
...
static void mNpc_GetEventPresent(mActor_name_t* present, int type) {
  static int priority_table[mNpc_EVENT_MAIL_FRIEND_NUM] = {
    mSP_LISTTYPE_RARE,
    mSP_LISTTYPE_UNCOMMON,
    mSP_LISTTYPE_COMMON
  };

  static int category_table[mNpc_EVENT_MAIL_FRIEND_NUM] = {
    mSP_KIND_FURNITURE,
    mSP_KIND_FURNITURE,
    mSP_KIND_CLOTH
  };
...
static void mNpc_GetEventMail(Mail_c* mail, PersonalID_c* pid, AnmPersonalID_c* anm_id, int type, int looks) {
  ...
  mNpc_LoadMailDataCommon2(mail, pid, anm_id, present, mNpc_GetPaperType(), 0x60 + looks * mNpc_EVENT_MAIL_FRIEND_NUM + type);
}
...
static void mNpc_SendEventPresentMailSex(int* selected, u8* type, Animal_c* animal, int animal_sex) {
  ...
  int other_sex_best_friend_idx = mNpc_GetAnimalMemoryFriend_Land_Sex(animal->memories, ANIMAL_MEMORY_NUM, (~animal_sex) & 1);
      ...
      if (other_sex_best_friends[j] >= 0 && types[j] == i) {
        ...
        if (((player_bitfield >> player_no) & 1) == 1) {
          if (mNpc_SendEventPresentMail(pid, player_no, &animal_p->id, i) == TRUE) {
            ...
            player_bitfield &= ~(1 << player_no); // remove player from bitfield because they can't receive letters
          }
        }
      }
...
extern int mNpc_CheckFriendship(PersonalID_c* pid, Animal_c* animal) {
  ...
  if (mNpc_CheckFreeAnimalPersonalID(&animal->id) == FALSE) {
    ...
    if (
      best_idx != -1 &&
      mPr_CheckCmpPersonalID(pid, &animal->memories[best_idx].memory_player_id) &&
...
static void mNpc_GetXmasCardData(Mail_c* mail, PersonalID_c* pid) {
  ...
  mHandbill_Load_HandbillFromRom(mail->content.header, &header_back_start, mail->content.footer, mail->content.body, 0xD7);
  mail->content.header_back_start = header_back_start;
  mail->content.font = mMl_FONT_0;
...
extern void mNpc_ChangePresentCloth() {
  ...
  for (i = 0; i < ANIMAL_NUM_MAX; i++) {
    ...
    if (
      mNpc_CheckFreeAnimalPersonalID(&animal->id) == FALSE &&
      present_cloth != EMPTY_NO &&
...
extern void mNpc_ResetWordEnding(ACTOR* actor) {
  if (actor != NULL && actor->part == ACTOR_PART_NPC) {
    ...
    if (animal != NULL) {
      mString_Load_StringFromRom(animal->catchphrase, ANIMAL_CATCHPHRASE_LEN, npc_def_list[animal->id.npc_id & 0xFFF].catchphrase_str_idx);
    }
  }
}
  ...
  if (looks < mNpc_LOOKS_NUM) {
    ...
    for (i = 0; i < NPC_NUM; i++) {
      if (looks == looks_tbl[0]) {
        ...
        if (mNpc_GetHaveAppeared_idx(i) == FALSE && (grow_type == mNpc_GROW_STARTER || grow_type == mNpc_GROW_MOVE_IN)) {
          num++;
        }
      }
...
static void mNpc_BuildHouseBeforeFieldct(mActor_name_t npc_id, int bx, int bz, int ut_x, int ut_z) {
  ...
  if (
    bx >= 0 && bx < FG_BLOCK_X_NUM &&
    bz >= 0 && bz < FG_BLOCK_Z_NUM &&
    ut_x > 0 && ut_x < (UT_X_NUM - 1) &&
...
static void mNpc_DestroyHouse(Anmhome_c* home) {
  ...
  if (bx < FG_BLOCK_X_NUM && bz < FG_BLOCK_Z_NUM && ut_x > 0 && ut_x < (UT_X_NUM - 1) && ut_z > 0 && ut_z < (UT_Z_NUM - 1)) {
...
static void mNpc_SetNpcHome(Animal_c* animal, Anmhome_c* reserved, u8 reserved_num) {
  ...
  if (reserved_num > 0) {
    ...
    for (i = 0; i < ANIMAL_NUM_MAX; i++) {
      if (ITEM_NAME_GET_TYPE(animal->id.npc_id) == NAME_TYPE_NPC && animal->home_info.block_x == 0xFF && animal->home_info.block_z == 0xFF) {
        idx = fake_table_p[n];
        ...
        if (idx < reserved_num) {
          ...
          mNpc_BuildHouseBeforeFieldct(animal->id.npc_id, animal->home_info.block_x - 1, animal->home_info.block_z - 1, animal->home_info.ut_x, home->ut_z);
        }
...
extern void mNpc_AddNpc_inBlock(mFM_move_actor_c* move_actor_list, u8 bx, u8 bz) {
  ...
  if ((scene == SCENE_NPC_HOUSE || scene == SCENE_KAMAKURA || scene == SCENE_COTTAGE_NPC) && move_actor_list != NULL) {
      ...
      if (actor->npc_id == animal->id.npc_id) {
        ...
        if (move_actor_list != NULL) {
          ...
          if (move_actor_idx != -1) {
            ...
            else if (mNpc_CheckNpcSet(bx, bz, ut_x, ut_z) == FALSE && mNpc_GetMakeUtNuminBlock_area(&ut_x, &ut_z, bx, bz, 0) == FALSE) {
              ut_x = animal->home_info.ut_x;
              ut_z = animal->home_info.ut_z;
            }
...
extern int mNpc_GetFriendAnimalNum(PersonalID_c* pid) {
  ...
  if (pid != NULL) {
    for (i = 0; i < ANIMAL_NUM_MAX; i++) {
      if (mNpc_CheckFreeAnimalPersonalID(&animal->id) == FALSE && mNpc_GetAnimalMemoryIdx(pid, animal->memories, ANIMAL_MEMORY_NUM) != -1) {
        num++;
      }
...
static int mNpc_CheckSelectFurniture(mActor_name_t item) {
  ...
  if (
    !(item >= FTR_CLOTH_START    && item <= FTR_CLOTH_END   ) && // Clothing (furniture)
  ...
  if (res1 > res0) {
    ...
    if (res1 > res0) {
      ...
      if (res1 > res0) {
        ...
        /* Finally, check the total number of animals in town with the same personality and pick the one with more */
...
static int mNpc_SendGoodbyAnimalMailOne(Mail_c* mail, PersonalID_c* pid, int player_no, Anm_GoodbyMail_c* goodby_mail) {
  ...
  if (mPr_NullCheckPersonalID(pid) == FALSE && mPr_CheckCmpPersonalID(pid, &home->ownerID) == TRUE) {
    ...
    else if (mPO_get_keep_mail_sum() < mPO_MAIL_STORAGE_SIZE && mNpc_SetGoodbyMailData(mail, pid, &goodby_mail->id) == TRUE) {
      res = mPO_receipt_proc(mail, mPO_SENDTYPE_MAIL);
    }
  }
  ...
  if (mLd_PlayerManKindCheck() == FALSE && mNpc_CheckFreeAnimalInfo(return_animal) == FALSE) {
    if (return_animal->removing == FALSE) {
      ...
      if (found_animal != NULL) {
        ...
        if (
          mNpc_CheckBuildHouse(
            home_info->block_x - 1, home_info->block_z - 1,
            home_info->ut_x, home_info->ut_z - 1
        ) == TRUE) {
          mNpc_BuildHouseBeforeFieldct(
            found_animal->id.npc_id,
            home_info->block_x - 1, home_info->block_z - 1,
            home_info->ut_x, home_info->ut_z - 1
          );
        }
...
extern int mNpc_GetNpcSoundSpec(ACTOR* actor) {
  static int spec_table[mNpc_LOOKS_NUM] = {
    4,
    4,
    2,
    2,
...
static int mNpc_CheckGrowFieldRank() {
  static int npc_grow_prob[mFAs_FIELDRANK_NUM] = {
    40,
    50,
    60,
    70,
...
static int mNpc_CheckGrow() {
  ...
  if (Save_Get(now_npc_max) < ANIMAL_NUM_MAX) {
    if (lbRTC_IsEqualTime(&mTM_rtcTime_clear_code, last_grow_time, lbRTC_CHECK_ALL) == TRUE) {
      ...
      if (
        interval >= (lbRTC_HOURS_PER_DAY * lbRTC_MINUTES_PER_HOUR) && /* Only one new villager per day */
        mLd_PlayerManKindCheck() == FALSE && /* Player must be from the town being loaded */
        mNpc_CheckFriendAllAnimal(&Common_Get(now_private)->player_ID) == TRUE /* The player loading must have talked to all current villagers */
      ) {
        res = TRUE;
      }
    }
    ...
    for (i = 0; i < ARRAY_COUNT(candidate_table); i++) {
      for (j = 0; j < 8; j++) {
        if (looks == *looks_table) {
          ...
          if (
            (mNpc_SearchAnimalinfo(Save_Get(animals), npc_idx | NPC_START, ANIMAL_NUM_MAX) == -1) &&
...
extern void mNpc_ForceRemove() {
  ...
  if (
    animal_num == ANIMAL_NUM_MAX &&
    force_remove_date->year != 0xFFFF &&
    force_remove_date->month != 0xFF &&
  ...
  if (free_mask_idx != -1) {
    ...
    if (ITEM_NAME_GET_TYPE(npc_id) == NAME_TYPE_NPC) {
      ...
      if (Common_Get(npc_is_summercamper) == TRUE) {
        ...
        mNpc_SetAnimalMemory(&Common_Get(now_private)->player_ID, &mask_npc->animal_data.id, &mask_npc->animal_data.memories[0]);
      }
...
static int mNpc_CheckNpcSet_fgcol_hard(mActor_name_t fg_item, u32 attribute) {
  ...
  if (mFI_CheckFGNpcOn(fg_item) && (attribute <= mCoBG_ATTRIBUTE_GRASS2 || (attribute >= mCoBG_ATTRIBUTE_SOIL0 && attribute <= mCoBG_ATTRIBUTE_SOIL2) || attribute == mCoBG_ATTRIBUTE_STONE)) {
    res = TRUE;
  }
...
extern int mNpc_CheckNpcSet(int bx, int bz, int ut_x, int ut_z) {
  ...
  if (collision != NULL && items != NULL) {
    res = mNpc_CheckNpcSet_fgcol(items[ut_z * UT_X_NUM + ut_x], collision[ut_z * UT_X_NUM + ut_x].data.unit_attribute);
  }
    ...
    for (i = restrict_area; i < UT_Z_NUM - restrict_area; i++) {
      for (j = restrict_area; j < UT_X_NUM - restrict_area; j++) {
        ...
        if (mNpc_CheckNpcSet_fgcol_hard(items[i * UT_X_NUM + j], col_p->data.unit_attribute) == TRUE) {
          ...
          if (
            (int)center == col_p->data.top_left &&
            center == col_p->data.bot_left &&
            center == col_p->data.top_right &&
            (int)center == col_p->data.bot_right
          ) {
...
extern int mNpc_GetMakeUtNuminBlock_area(int* ut_x, int* ut_z, int bx, int bz, int restrict_area) {
  ...
  if (fg_top != NULL && col_top != NULL) {
    for (i = restrict_area; i < UT_Z_NUM - restrict_area; i++) {
      for (j = restrict_area; j < UT_X_NUM - restrict_area; j++) {
        if (mNpc_CheckNpcSet_fgcol(fg_top[i * UT_X_NUM + j], col_top[i * UT_X_NUM + j].data.unit_attribute) == TRUE) {
          *fg_flag_p |= 1 << j;
          candidates++;
        }
      }
      ...
      for (i = 0; i < UT_TOTAL_NUM; i++) {
        if (ITEM_IS_FTR(*island_room)) {
          ...
          for (j = 0; j < mRmTp_DIRECT_NUM; j++) {
            if (*island_room == (mActor_name_t)(ftr_variant0 + j) || *island_room == (mActor_name_t)(ftr_variant1 + j)) {
              idx = n;
              break;
            }
          }
    ...
    if (mNpc_CheckFtrIsIslandBestFtr(ftr) == TRUE) {
      ...
      for (i = 0; i < UT_TOTAL_NUM; i++) {
        if (ITEM_IS_FTR(*island_room)) {
          ...
          if (variant != -1 && n >= 0 && n < mNpc_ISLAND_FTR_NUM && ((island_ftr_p->set_ftr_bitfield >> n) & 1) == 0) {
...
static int mNpc_CheckIslandNpcRoomFtrIdx(int ftr_idx) {
  ...
  if (mNpc_CheckFreeAnimalInfo(animal) == FALSE && ftr_idx >= 0 && ftr_idx < mNpc_ISLAND_FTR_NUM) {
    ...
    for (i = 0; i < ANIMAL_MEMORY_NUM; i++) {
      if (
        mPr_NullCheckPersonalID(&memory->memory_player_id) == FALSE &&
        ...
        for (i = 0; i < UT_TOTAL_NUM; i++) {
          if (ITEM_IS_FTR(*island_room)) {
            ...
            if (variant != -1 && mNpc_CheckIslandNpcRoomFtrIdx(n) == -1) {
              ...
              if (slot < mNpc_ISLAND_FTR_SAVE_NUM) {
                Save_Get(island).animal.anmuni.island_ftr[slot] = aMR_FurnitureFg_to_FurnitureFgWithDirect(ftr, direct);
                set = TRUE;
              }
      ...
      if (rsv_no != EMPTY_NO) {
        for (i = 0; i < UT_TOTAL_NUM; i++) {
          if (ITEM_IS_FTR(*island_room)) {
            ...
            if (variant != -1 && n >= 0 && n < mNpc_ISLAND_FTR_NUM && ((island_ftr_p->set_ftr_bitfield >> n) & 1) == 1) {
              u16* set_ftr_bitfield_p = &island_ftr_p->set_ftr_bitfield;
        ...
        if (ITEM_IS_FTR(*island_room)) {
          if (mNpc_CheckIslandNpcRoomFtrIdx(n) != -1) {
            if (selected == 0) {
              ...
              present_ftr_p->present = aMR_FurnitureFg_to_FurnitureFgWithDirect(*island_room, mRmTp_DIRECT_SOUTH);
      ...
      for (i = 0; i < UT_TOTAL_NUM; i++) {
        if (ITEM_IS_FTR(*island_room)) {
          if (aMR_CorrespondFurniture(*island_room, exist_ftr) == FALSE) {
            ...
            if (ftr_idx != -1 && ftr_idx != mem_idx) {
              if (selected == 0) {
                other_best_ftr[0] = aMR_FurnitureFg_to_FurnitureFgWithDirect(*island_room, mRmTp_DIRECT_SOUTH);
...
static int mNpc_CheckIslandAnimalTableNo(mActor_name_t npc_id) {
  ...
  if (
    ITEM_NAME_GET_TYPE(npc_id) == NAME_TYPE_NPC &&
    (npc_id & 0xFFF) < NPC_NUM &&
...
static int mNpc_SendHPMailNum_famicom(PersonalID_c* pid, AnmPersonalID_c* anm_id, mMpswd_password_c* password) {
  ...
  if (mPO_get_keep_mail_sum() < mPO_MAIL_STORAGE_SIZE) {
    ...
    if (
      password->hit_rate_index == 1 && // hit rate index must be 1
  ...
  if (mPO_get_keep_mail_sum() < mPO_MAIL_STORAGE_SIZE) {
    ...
    if (
      ...
      if (password->npc_type == mMpswd_NPCTYPE_NORMAL) {
        ...
        if (
          (villager_idx != -1 || islander_idx != -1) &&
          (
            mNpc_GetAnimalMemoryIdx(&Common_Get(now_private)->player_ID, Save_Get(animals[villager_idx]).memories, ANIMAL_MEMORY_NUM) != -1 ||
            mNpc_GetAnimalMemoryIdx(&Common_Get(now_private)->player_ID, Save_Get(island).animal.memories, ANIMAL_MEMORY_NUM) != -1
          )
        ) {
...
static int mNpc_SendHPMailNum_user_password(PersonalID_c* pid, AnmPersonalID_c* anm_id, mMpswd_password_c* password) {
  if (
    mMpswd_password_zuru_check(password) == FALSE &&
...
static int mNpc_SendHPMailNum_magazine(PersonalID_c* pid, AnmPersonalID_c* anm_id, mMpswd_password_c* password) {
  ...
  if (mPO_get_keep_mail_sum() < mPO_MAIL_STORAGE_SIZE) {
    ...
    if (
      mNpc_CheckHit_Rate(password->hit_rate_index, 3) == TRUE && // hitrate index is valid
      mMpswd_password_zuru_check(password) == FALSE && // checksum is valid
      mMpswd_check_present(password) == TRUE // present is valid
    ) {
      /* Valid password */
...
static int mNpc_SendHPMail_analysis(PersonalID_c* pid, AnmPersonalID_c* anm_id, AnmHPMail_c* hp_mail) {
  static mNpc_HPMAIL_SEND_PROC send_proc[mMpswd_CODETYPE_NUM] = {
    &mNpc_SendHPMailNum_famicom,
    &mNpc_SendHPMailNum_popular,
    &mNpc_SendHPMailNum_cardE,
    &mNpc_SendHPMailNum_magazine,
    &mNpc_SendHPMailNum_user_password,
    &mNpc_SendHPMailNum_cardE_mini
  };
  ...
  for (i = 0; i < ANIMAL_NUM_MAX; i++) {
    if (mNpc_CheckFreeAnimalPersonalID(&animal->id) == FALSE) {
      ...
      for (j = 0; j < ANIMAL_HP_MAIL_NUM; j++) {
        if (hp_mail->receive_time.year != 0 && hp_mail->receive_time.day != 0 && mPr_NullCheckPersonalID(&priv_p->player_ID) == FALSE) {
          int days;
...
extern void mNpc_PrintFriendship_fdebug(gfxprint_t* gfxprint) {
  static mNpc_PRINT_FRIENDSHIP_PROC print_proc[2] = {
    &mNpc_PrintFriendship_fdebug_0,

...
extern int mRmTp_GetFloorIdx() {
  ...
  if (mSc_IS_SCENE_BASEMENT(scene_no)) {
    ...
    Save_Get(scene_no) == SCENE_MY_ROOM_S ||
    Save_Get(scene_no) == SCENE_MY_ROOM_M ||
    Save_Get(scene_no) == SCENE_MY_ROOM_L ||
...
extern int mRmTp_GetWallIdx() {
  ...
  if (
    scene_no == SCENE_MY_ROOM_S ||
    scene_no == SCENE_MY_ROOM_M ||
    scene_no == SCENE_MY_ROOM_L ||
    scene_no == SCENE_MY_ROOM_LL1 ||
    scene_no == SCENE_MY_ROOM_LL2
  ) {
    mActor_name_t field_id = mFI_GetFieldId();
    int floor_no = mFI_GetNowPlayerHouseFloorNo();
    int idx = field_id - mFI_FIELD_PLAYER0_ROOM;
...
extern void mRmTp_CopyWallData(u8* dst, int wall_no) {
  if (dst != NULL) {
    if ((wall_no < 72 || wall_no > 79) && (wall_no < 0 || wall_no >= 71)) {
      wall_no = 0;
    }

    if (wall_no >= 72 && wall_no <= 79) {
      int valid = wall_no >= 72 && wall_no <= 79;
      int tex_idx = valid ? (wall_no - 72) & 7 : -1;
...
extern void mRmTp_CopyFloorData(u8* dst, int floor_no) {
  if (dst != NULL) {
    if ((floor_no < 87 || floor_no > 94) && (floor_no < 0 || floor_no >= 71)) {
      floor_no = 0;
    }

    if (floor_no >= 87 && floor_no <= 94) {
      int valid = floor_no >= 87 && floor_no <= 94;
      int tex_idx = valid ? (floor_no - 87) & 7 : -1;
      int room_idx = mRmTp_GetPlayerRoomIdx();
      int valid2 = mHS_get_pl_no_detail(room_idx) >= 0 && mHS_get_pl_no_detail(room_idx) < PLAYER_NUM;
      int player_no = valid2 ? mHS_get_pl_no_detail(room_idx) : 0;
      mNW_original_design_c* original = &Save_Get(private[player_no]).my_org[tex_idx & 7];
    ...
    else if (floor_no >= 0 && floor_no < 71) {
...
extern int mRmTp_CheckTalkAble(s16* rot, xyz_t* pos, f32 max_dist, int required_dir, GAME_PLAY* play) {
  static int direct_table[16] = {
    mRmTp_DIRECT_NORTH, -1, -1,  mRmTp_DIRECT_WEST,
...
extern int mRmTp_GetNowSceneLightSwitchIndex() {
  ...
  if (scene == SCENE_LIGHTHOUSE) {
    ...
    int base = mFI_GetNowPlayerHouseFloorNo() == 2 ? mRmTp_LIGHT_SWITCH_HOUSE0_BASEMENT : mRmTp_LIGHT_SWITCH_HOUSE0_MAIN_UPPER;
...
extern int mRmTp_Index2LightSwitchStatus(int switch_index) {
  return (switch_index >= 0 && switch_index < mRmTp_LIGHT_SWITCH_NUM) ? ((l_mRmTp_switch_data >> switch_index) & 1) : TRUE;
}
...
static int mRmTp_CheckMyRoomLightON(int house_idx, int check_owned) {
  ...
  if (check_owned == TRUE) {
    if (mRmTp_CheckExistMyRoomOwner(house_idx) && (now_sec >= (18 * mTM_SECONDS_IN_HOUR) || now_sec < (5 * mTM_SECONDS_IN_HOUR))) {
      return TRUE;
    }
  }

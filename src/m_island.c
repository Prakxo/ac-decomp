...
static u8 l_misl_count_max_table[32] = {
  10,
  4,
  1,
  10,
...
static void mISL_RestoreIslandComb() {
  ...
  if (island_x_blocks[0] > 0 && island_x_blocks[1] < (BLOCK_X_NUM - 1)) {
    mISL_RestoreIslandComb_com(Save_GetPointer(combi_table[mISL_BLOCK_Z][island_x_blocks[0]]), l_keepIslandComb, mISL_FG_BLOCK_X_NUM);
  }
}
...
extern void mISL_ChangeBG() {
  ...
  if (island_x_blocks[0] > 0 && island_x_blocks[1] < (BLOCK_X_NUM - 1)) {
    mISL_KeepIslandComb(l_keepIslandComb, Save_GetPointer(combi_table[mISL_BLOCK_Z][island_x_blocks[0]]), mISL_FG_BLOCK_X_NUM);
...
static void mISL_gc_to_agb_layer(mHm_lyr_c* agb, mHm_lyr_c* gc) {
  ...
  for (i = 0; i < mHm_LAYER_NUM; i++) {
    ...
    mISL_int(agb->unk_208, gc->unk_208, ARRAY_COUNT(agb->unk_208));
  ...
  if (agb != NULL) {
    ...
    if (mFI_CheckFieldData() == TRUE) {
      ...
      for (i = 0; i < mISL_FG_BLOCK_X_NUM; i++) {
        ...
        u16* deposit_p = (u16*)&gc->deposit[(u16)i]; // the access to deposit is strange, casting to u16 is incorrect but it makes all other instructions correct

...
static u8* kei_win_st_tex_tbl[] = {
  kei_win_st5_tex_rgb_ia8,
  kei_win_st4_tex_rgb_ia8,
  kei_win_st2_tex_rgb_ia8,
  kei_win_st4_tex_rgb_ia8,
  kei_win_st5_tex_rgb_ia8,
  kei_win_st6_tex_rgb_ia8,
  kei_win_st3_tex_rgb_ia8,
  kei_win_st6_tex_rgb_ia8,
...
static void mNT_Play_page_read(Submenu* submenu, mSM_MenuInfo_c* menu_info, mNT_Ovl_c* notice_ovl) {
  ...
  if (notice_ovl->now_page != 0 && (stick_area == mED_STICK_AREA_LEFT || chkTrigger(BUTTON_CLEFT))) {
  ...
  else if (notice_ovl->now_page < (notice_ovl->page_count - 1) && (stick_area == mED_STICK_AREA_RIGHT || chkTrigger(BUTTON_CRIGHT))) {
    move_page = 1;
  }
  else if (notice_ovl->now_page != 0 && (stick_area == mED_STICK_AREA_BOTTOM || chkTrigger(BUTTON_CDOWN))) {
    move_page = -notice_ovl->now_page;
  }
  else if (notice_ovl->now_page < (notice_ovl->page_count - 1) && (stick_area == mED_STICK_AREA_TOP || chkTrigger(BUTTON_CUP))) {
    ...
    mSM_open_submenu_new2(submenu, mSM_OVL_EDITOR, mED_TYPE_NOTICE, 32, notice_ovl->post.message, sizeof(notice_ovl->post.message));
  }
...
static void mNT_move_Play(Submenu* submenu, mSM_MenuInfo_c* menu_info) {
  static mNT_MOVE_PROC move_proc[mNT_PLAY_PAGE_NUM] = {
    &mNT_Play_page_read,
    &mNT_Play_page_move,
...
static void mNT_move_Obey(Submenu* submenu, mSM_MenuInfo_c* menu_info) {
  ...
  if (editEndChk_menu->proc_status == mSM_OVL_PROC_MOVE && editEndChk_menu->next_proc_status == mSM_OVL_PROC_END) {
    if (editEndChk_menu->data1 == 0) {
      ...
      mSM_open_submenu_new2(submenu, mSM_OVL_EDITOR, mED_TYPE_NOTICE, 0, notice_ovl->post.message, sizeof(notice_ovl->post.message));
...
static void mNT_notice_ovl_move(Submenu* submenu) {
  static mNT_OVL_MOVE_PROC ovl_move_proc[mSM_OVL_PROC_NUM] = {
    &mNT_move_Move,
    &mNT_move_Play,
    &mNT_move_Wait,
    &mNT_move_Obey,
...
static void mNT_set_day_strings_dl(GAME* game, lbRTC_time_c* time_p, f32 x, f32 y) {
  static mNT_month_str_c month_str[lbRTC_MONTHS_MAX + 1] = {
    mNT_MONTH_STR(january_str),
    mNT_MONTH_STR(february_str),
    mNT_MONTH_STR(march_str),
    mNT_MONTH_STR(april_str),
    mNT_MONTH_STR(may_str),
    mNT_MONTH_STR(june_str),
    mNT_MONTH_STR(july_str),
    mNT_MONTH_STR(august_str),
    mNT_MONTH_STR(september_str),
    mNT_MONTH_STR(october_str),
    mNT_MONTH_STR(november_str),
    mNT_MONTH_STR(december_str),
...
static void mNT_set_strings_dl(mSM_MenuInfo_c* menu_info, GAME* game, u8* str_p, int body_len, f32 x, f32 y, f32* end_x, f32* end_y) {
  u8* end_p;
  int i;
  int j;
  int width;
  ...
  for (i = 0; i < mNT_MAX_LINES; i++) {
    ...
    do {
      ...
      if (line_chk == mNT_LINE_CHECK_OVERSTRING) {
        if (
          i != (mNT_MAX_LINES - 1) &&
          ((now_str_p != original_str_p && now_str_p[-1] == CHAR_NEW_LINE) || width + mFont_GetCodeWidth(*now_str_p, TRUE) > mNT_MAX_WIDTH)
        ) {
...
static void mNT_set_page_dl(Submenu* submenu, mSM_MenuInfo_c* menu_info, GAME* game, GRAPH* graph, f32 x, f32 y, int page_no) {
  mNT_Ovl_c* notice_ovl = submenu->overlay->notice_ovl;
  mED_Ovl_c* editor_ovl = submenu->overlay->editor_ovl;
  ...
  if (menu_info->proc_status == mSM_OVL_PROC_WAIT && editor_ovl != NULL) {
    (*submenu->overlay->editor_ovl->cursol_draw)(submenu, game, x + (f32)editor_ovl->_26 + -7.0f, y + editor_ovl->_24 * 16.0f);

...
static void mNtc_operate_data_list() {
  ...
  mNtc_auto_nwrite_data[0x29].date = mEv_weekday2day(lbRTC_MARCH, mEv_WEEKTYPE_LAST, lbRTC_SUNDAY) + 0x300; /* month = march, day = last sunday */
  mNtc_auto_nwrite_data[0x2A].date = mEv_weekday2day(lbRTC_OCTOBER, mEv_WEEKTYPE_LAST, lbRTC_SUNDAY) + 0x9F9; /* month = october, day = second to last sunday */
}
  ...
  if (rtc_time.hour >= mTM_FIELD_RENEW_HOUR) {
    ...
    if (lbRTC_IsEqualTime(treasure_buried_time, &mTM_rtcTime_clear_code, lbRTC_CHECK_ALL) ||
      ...
      if (lbRTC_IsEqualTime(treasure_checked_time, &mTM_rtcTime_clear_code, lbRTC_CHECK_ALL) ||
          treasure_checked_time->year != rtc_time.year ||
          treasure_checked_time->month != rtc_time.month ||
          ...
          if (mNpc_CheckFreeAnimalPersonalID(&animal->id) == FALSE) {
            ...
            for (mem = 0; mem < ANIMAL_MEMORY_NUM; mem++) {
              if (animal->memories[mem].memory_player_id.land_id == land_info->id &&
                  mLd_CheckCmpLandName(animal->memories[mem].memory_player_id.land_name, land_info->name)
              ) {
        ...
        if (valid_animal_num != 0 && fqrand() < mNtc_TREASURE_CHANCE) {
          ...
          if (rng < (1.0f / 3.0f)) {
            ...
            int list_type = rng < (2.0f / 3.0f) ? mSP_LISTTYPE_LOTTERY : mSP_LISTTYPE_EVENT; /* 50/50 to roll redd or lottery furniture*/
          ...
          if (mFI_SetTreasure(&b_x, &b_z, item)) {
            ...
            int header_back_pos;
            mNtc_board_post_c post;
            int selected_animal = (int)(fqrand() * (f32)valid_animal_num);
            int selected_animal_idx = valid_animal_ids[selected_animal % valid_animal_num];
            animal = Save_GetPointer(animals[selected_animal_idx]);

            mNtc_set_treasure_string(&animal->id, item, b_x, b_z);
            lbRTC_TimeCopy(&post.post_time, &rtc_time);
            mHandbill_Load_HandbillFromRom(header, &header_back_pos, footer, post.message, 0x1F0 + animal->id.looks * 3 + (((int)(fqrand() * 3.0f)) % 3));
            mNtc_notice_write(&post);
            lbRTC_TimeCopy(Save_GetPointer(treasure_buried_time), &rtc_time);
          }
        }
      }
    }
...
extern void mNtc_auto_nwrite_time_ct() {
  ...
  if (lbRTC_IsEqualTime(Save_GetPointer(saved_auto_nwrite_time), &mTM_rtcTime_clear_code, lbRTC_CHECK_ALL) == TRUE) {
    ...
    mNtc_set_near_old_nwrite_data(&rtc_time, Common_GetPointer(auto_nwrite_year), Common_GetPointer(auto_nwrite_count));
  }
  else {
    mNtc_set_near_old_nwrite_data(Save_GetPointer(save_check.time), Common_GetPointer(auto_nwrite_year), Common_GetPointer(auto_nwrite_count));
  }
}
...
static int mNtc_get_fishing_day(lbRTC_ymd_t* ymds, u16* dates, lbRTC_time_c* time, u16* fishing_term_date) {
  ...
  for (i = 0; i < 5; i++) {
    ...
    if (ymd->year > notice_year  ||
       (notice_year == ymd->year && (month_day > *fishing_term_date ||
       (month_day == *fishing_term_date && notice_hour < 18)))
    ) {
...
// @nonmatching
/* TODO: match */
static void mNtc_decide_nwrite_data(
  int* write_count,
  lbRTC_year_t* nwrite_year,
  u8* nwrite_num,
  lbRTC_year_t now_nwrite_year,
  u8 now_nwrite_num,
  lbRTC_year_t* out_nwrite_years,
  u8* out_nwrite_nums,
  int* fishing_day,
  ...
  while (*write_count != 0) {
    while (TRUE) {
      //fish_day = *fishing_day;
      if ((*fishing_day <= 0 || (ymds[*fishing_day - 1].year <= fishing_term_year && (fishing_term_year != ymds[*fishing_day - 1].year || (month_days[*fishing_day - 1] < fishing_term_date)))) ||
          (ymds[*fishing_day - 1].year <= *nwrite_year && (*nwrite_year != ymds[*fishing_day - 1].year || month_days[*fishing_day - 1] < mNtc_auto_nwrite_data[*nwrite_num].date))
      ) {
        break;
      }
      ...
      out_nwrite_years[*write_count] = -1;
      out_nwrite_nums[*write_count] = -1;
    }

    if (*nwrite_year <= now_nwrite_year) {
      if (*nwrite_year != now_nwrite_year) {
        return;
      }
      if (*nwrite_num <= now_nwrite_num) {
        return;
      }
    }
...
static void mNtc_set_auto_nwrite(int write_count, u8* nwrite_nums, lbRTC_year_t* nwrite_years, int fishing_day, lbRTC_ymd_t* ymds, u16* month_days) {
  ...
  for (write_count; write_count < 5; write_count++) {
    ...
    if (nwrite_nums[write_count] == mNtc_FISHING_DAY_NUM && nwrite_years[write_count] == mNtc_FISHING_DAY_YEAR) {
      ...
      post.post_time.day = mNtc_GET_DAY(month_days[fishing_day]);
      post.post_time.weekday = lbRTC_Week(post.post_time.year, post.post_time.month, post.post_time.day);

      temp = *post_date;
      mNtc_set_auto_nwrite_fishing_string(&temp);
      now_year = 0;
      mHandbill_Load_HandbillFromRom(header, &header_back_pos, footer, post.message, 0x242 + ((post_date->day / 7u) & 3));
      mNtc_notice_write(&post);
    }
      ...
      mHandbill_Load_HandbillFromRom(header, &header_back_pos, footer, post.message, 0x1A4 + mNtc_auto_nwrite_data[nwrite_nums[write_count]].id);
      mNtc_notice_write(&post);
    }
  }
}
  ...
  if (mEv_CheckTitleDemo() == FALSE && mFI_GET_TYPE(mFI_GetFieldId()) == mFI_TYPE(mFI_FIELDTYPE_FG)) {
    ...
    if (now_nwrite_year != 0) {
      ...
      if (write_count != mNtc_NWRITE_NUM) {
        mNtc_set_auto_nwrite(write_count, set_nwrite_nums, set_nwrite_years, fishing_day, nwrite_ymds, nwrite_dates);
      }
      else {

...
static mActor_name_t mPr_boy_cloth_table[8] = { ITM_CLOTH000, ITM_CLOTH001, ITM_CLOTH002, ITM_CLOTH003, ITM_CLOTH004, ITM_CLOTH005, ITM_CLOTH006, ITM_CLOTH007 };
static mActor_name_t mPr_girl_cloth_table[8] = { ITM_CLOTH008, ITM_CLOTH009, ITM_CLOTH010, ITM_CLOTH011, ITM_CLOTH012, ITM_CLOTH013, ITM_CLOTH014, ITM_CLOTH015 };
...
extern int mPr_GetPossessionItemIdxFGTypeWithCond_cancel(Private_c* priv, mActor_name_t fg_type, u32 cond, mActor_name_t cancel_item) {
  ...
  if (priv != NULL) {
    ...
    for (i = 0; i < mPr_POCKETS_SLOT_COUNT; i++) {
      if (ITEM_NAME_GET_TYPE(*pockets) == fg_type && cond == mPr_GET_ITEM_COND(priv->inventory.item_conditions, i) && *pockets != cancel_item) {
        idx = i;
        break;
      }
...
extern int mPr_GetPossessionItemIdxItem1CategoryWithCond_cancel(Private_c* priv, u8 item1_type, u32 cond, mActor_name_t cancel_item) {
  ...
  if (priv != NULL) {
    ...
    for (i = 0; i < mPr_POCKETS_SLOT_COUNT; i++) {
      if (ITEM_NAME_GET_TYPE(*pockets) == NAME_TYPE_ITEM1 && GET_NAME_ITEM1_CATEGORY(*pockets) == item1_type && cond == mPr_GET_ITEM_COND(priv->inventory.item_conditions, i) && *pockets != cancel_item) {
        idx = i;
        break;
      }
      pockets++;
...
extern int mPr_GetPossessionItemIdxKindWithCond(Private_c* priv, mActor_name_t kind_start, mActor_name_t kind_end, u32 cond) {
  ...
  if (priv != NULL) {
    ...
    for (i = 0; i < mPr_POCKETS_SLOT_COUNT; i++) {
      if (*pockets >= kind_start && *pockets < kind_end && cond == mPr_GET_ITEM_COND(priv->inventory.item_conditions, i)) {
        idx = i;
        break;
      }
      pockets++;
...
extern int mPr_GetPossessionItemSumItemCategoryWithCond(Private_c* priv, u8 item1_type, u32 cond) {
  ...
  if (priv != NULL) {
    ...
    for (i = 0; i < mPr_POCKETS_SLOT_COUNT; i++) {
      if (ITEM_NAME_GET_TYPE(*pockets) == NAME_TYPE_ITEM1 && GET_NAME_ITEM1_CATEGORY(*pockets) == item1_type && cond == mPr_GET_ITEM_COND(priv->inventory.item_conditions, i)) {
        sum++;
      }
      pockets++;
    }
...
extern void mPr_SetItemCollectBit(mActor_name_t item) {
  ...
  if ((ftr_item_no >= FTR_FISH00 && ftr_item_no <= FTR_FISH39_WEST) || (ftr_item_no >= FTR_INSECT00 && ftr_item_no <= FTR_INSECT39_WEST)) {
    return;
  }
  ...
  if (item_type == NAME_TYPE_FTR0 || item_type == NAME_TYPE_FTR1) {
        ...
        if (category == ITEM1_CAT_PAPER) {
          ...
          mActor_name_t idx = ftr_item_no - ITM_MINIDISK_START; // why is this one u16, probably mActor_name_t type or something?
...
extern void mPr_SetPossessionItem(Private_c* priv, int idx, mActor_name_t item, u32 cond) {
  ...
  priv->inventory.item_conditions = ((priv->inventory.item_conditions & (~(u32)(mPr_ITEM_COND_NUM << (idx * 2)))) | (cond << (idx * 2)));
...
static int mPr_GetMotherMailPaperType(int month, int day) {
  static u8 paper_table[lbRTC_MONTHS_MAX] = {
    13, 49, 32, 12, 62, 14, 19, 11, 59, 46, 47, 17
  };
...
static void mPr_GetMotherMail(Mail_c* mail, PersonalID_c* pid, mActor_name_t present, int stationery, int mail_no) {
  ...
  mMl_clear_mail(mail);
  mHandbill_Load_HandbillFromRom(mail->content.header, &header_back_pos, mail->content.footer, mail->content.body, mail_no);
...
static int mPr_SendMotherMailPost(PersonalID_c* pid, int player_no, mActor_name_t present, int stationery, int mail_no) {
  Mail_c* mail;
  mHm_hs_c* home;
  int res = FALSE;
...
static int mPr_SendMotherMailDate(mPr_mother_mail_info_c* mother_mail, lbRTC_time_c* send_time) {
  ...
  if (Common_Get(now_private)->birthday.month == send_time->month && Common_Get(now_private)->birthday.day == send_time->day) {
  ...
  if (mail_no != -1) {
    if (mPr_SendMotherMailPost(&Common_Get(now_private)->player_ID, Common_Get(player_no), present, mPr_GetMotherMailPaperType(send_time->month, send_time->day), mail_no) == TRUE) {
...
static void mPr_GetMotherMailNormalData(mPr_mother_mail_data_c* send_data, int* mail_no, mActor_name_t* present, int* event_no, int no_send_num) {
  int selected_event;
  int i;
...
static void mPr_SendMotherMailNormal(mPr_mother_mail_info_c* mother_mail, lbRTC_time_c* send_time) {
  ...
  if (RANDOM(100) < 20) {
    ...
    if (monthly_not_send_num > 0) {
      mPr_GetMotherMailMonthlyData(&mother_mail->data, &mail_no, &present, &event_no, send_time->month, monthly_not_send_num);
...
extern void mPr_SendForeingerAnimalMail(Private_c* priv) {
  ...
  if (mLd_PlayerManKindCheck() == FALSE) {
    if (anm_mem->npc_id != RSV_NO && ITEM_NAME_GET_TYPE(anm_mem->npc_id) == NAME_TYPE_NPC && mPO_get_keep_mail_sum() < mPO_MAIL_STORAGE_SIZE) {
      mMl_clear_mail(mail);
...
extern int mPr_CheckFishCompleteTalk(u8 player_no) {
  ...
  if (player_no < mPr_FOREIGNER && mPr_GetCompleteTalk(Save_Get(private[player_no]).complete_fish_insect_flags, 0) == TRUE) {
    res = TRUE;
  }
...
extern int mPr_CheckInsectCompleteTalk(u8 player_no) {
  ...
  if (player_no < mPr_FOREIGNER && mPr_GetCompleteTalk(Save_Get(private[player_no]).complete_fish_insect_flags, 1) == TRUE) {
    res = TRUE;
  }

...
static lbRTC_day_t l_delivery_limit[4] = {
  2,
  2,
  2,
  2
...
static lbRTC_day_t l_errand_limit[mQst_ERRAND_NUM] = {
  2,
  2,
  2,
  0,
...
static lbRTC_day_t* l_limit_table[mQst_QUEST_TYPE_NUM] = {
  l_delivery_limit,
  l_errand_limit,
  l_contest_limit
};
...
extern int mQst_GetOccuredDeliveryIdx(int delivery_kind) {
  ...
  for (i = 0; i < mPr_DELIVERY_QUEST_NUM; i++) {
    if (mQst_CheckFreeQuest(&delivery->base) == FALSE && delivery->base.quest_type == mQst_QUEST_TYPE_DELIVERY && (u32)delivery->base.quest_kind == delivery_kind) {
      idx = i;
      break;
    }
...
static int mQst_GetDeliveryIdxbyItemIdx(int idx) {
  ...
  if (idx >= 0 && idx < mPr_POCKETS_SLOT_COUNT && mQst_CheckFreeQuest(&Common_Get(now_private)->deliveries[idx].base) == FALSE) {
    d_idx = idx;
  }
  ...
  if (idx >= 0 && idx < mPr_POCKETS_SLOT_COUNT) {
    ...
    if (item != EMPTY_NO) {
      ...
      if (res == FALSE) {
        ...
        for (i = 0; i < mPr_ERRAND_QUEST_NUM; i++) {
          if (mQst_CheckFreeQuest(&errand->base) == FALSE && errand->pockets_idx == idx && item == errand->item && mQst_CheckLimitOver(&errand->base) == TRUE) {
            res = TRUE;
            break;
          }
...
extern void mQst_CheckGrabItem(mActor_name_t item, int pocket_idx) {
  ...
  if (pocket_idx >= 0 && pocket_idx < mPr_POCKETS_SLOT_COUNT) {
    ...
    if ((u32)grab->type == mQst_QUEST_TYPE_NONE && mPr_GET_ITEM_COND(item_cond, pocket_idx) == mPr_ITEM_COND_QUEST) {

      for (i = 0; i < mPr_ERRAND_QUEST_NUM; i++) {
        if (mQst_CheckFreeQuest(&errand->base) == FALSE && errand->pockets_idx == pocket_idx && errand->item == item) {
          grab->item = item;
          grab->pocket_idx = i;
...
extern int mQst_GetOccuredContestIdx(int kind) {
  ...
  for (i = 0; i < ANIMAL_NUM_MAX; i++) {
    if (animal->contest_quest.base.quest_type == mQst_QUEST_TYPE_CONTEST && (u32)animal->contest_quest.base.quest_kind == kind) {
      res = i;
      break;
    }
...
static void mQst_GetRemailData(Mail_c* letter, PersonalID_c* recipient_id, AnmPersonalID_c* sender_id, int rank, mActor_name_t present) {
  int handbill_no;
  ...
  mHandbill_Load_HandbillFromRom2(header, MAIL_HEADER2_LEN, &header_back_pos, footer, MAIL_FOOTER2_LEN, letter->content.body, handbill_no);
    ...
    if (priv_idx != -1) {
      ...
      if (mPr_CheckCmpPersonalID(recipient_id, &house->ownerID) == TRUE) {
        ...
        if (free_mail_idx != -1) {
          ...
          mQst_GetRemailData(&letter, recipient_id, sender_id, contest->info.letter_data.score, contest->info.letter_data.present);
...
extern void mQst_SetReceiveLetter(mQst_contest_c* contest, PersonalID_c* sender_id, u8* body, mActor_name_t present) {
  if (
    ...
    contest->base.quest_kind == mQst_CONTEST_KIND_LETTER &&
    mPr_NullCheckPersonalID(&contest->player_id) == TRUE &&
...
extern mQst_errand_c* mQst_GetFirstJobData() {
  ...
  for (i = 0; i < mPr_ERRAND_QUEST_NUM; i++) {
    if (errand_p->base.quest_type == mQst_QUEST_TYPE_ERRAND && errand_p->errand_type == mQst_ERRAND_TYPE_FIRST_JOB) {
...
static int mQst_CheckFirstJobQuest(mQst_errand_c* errand) {
  if (errand != NULL && errand->base.quest_type == mQst_QUEST_TYPE_ERRAND && errand->errand_type == mQst_ERRAND_TYPE_FIRST_JOB) {
    return TRUE;
  }
...
static void mQst_SetFirstJobAxe_common(mQst_errand_c* errand, AnmPersonalID_c* pid, mActor_name_t item, u8 slot, u8 kind) {
  if (errand != NULL) {
    errand->base.quest_type = mQst_QUEST_TYPE_ERRAND;
...
extern void mQst_NextSnowman(xyz_t snowman_pos) {
  ...
  if (npc_idx != -1) {
    ...
    if (contest->base.progress == 1) {
      if (mFI_Wpos2BlockNum(&block_x, &block_z, snowman_pos) == TRUE && animal->home_info.block_x == block_x && animal->home_info.block_z == block_z) {
        mPr_CopyPersonalID(&contest->player_id, &Common_Get(now_private)->player_ID);
      }
    }
  }
